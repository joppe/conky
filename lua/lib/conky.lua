...
function setup_ring(cr, config)
	local value = config.data.value or calc_percent(get_value(config.data.prop, config.data.arg), config.data.max)
...
function setup_text(cr, config)
	local text = config.data.value or get_value(config.data.prop, config.data.arg)
